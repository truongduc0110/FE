...
<template>
  <h1 class="text-2xl font-bold text-color-dark max-md:text-xl max-lg:text-center">Mã giảm giá</h1>
  <div class="mt-8 md:container max-md:px-2 mx-auto product">
    ...
    <RouterLink :to="{ name : 'discounts-category'}">
      <button class="px-5 font-semibold py-2 bg-color-13 text-sm text-color-white rounded-lg max-md:px-3 ml-5">Giảm giá theo danh mục</button>
    </RouterLink>
      ...
      <div class="overflow-x-auto w-full max-md:my-0 max-md:mx-auto">
        <table class="w-full border-collapse min-w-[600px]">
          ...
          <tr v-for="(item,index) in dataDiscounts">
            ...
            <td class="h-10 border-b border-color-light text-color-dark max-md:text-xs whitespace-nowrap">{{ item.start_date }}</td>
            <td class="h-10 border-b border-color-light text-color-dark max-md:text-xs whitespace-nowrap">{{ item.end_date }}</td>
            <td class="h-10 border-b border-color-light text-color-dark max-md:text-xs whitespace-nowrap">{{ item.max_uses }}</td>
            ...
            <td class="h-10 border-b border-color-light text-color-dark whitespace-nowrap">
              <RouterLink :to="{ name : 'edit-discount', params : {id : item.id}}">
...
import {deleteDiscount, getDiscountList} from "@/service/DiscountsServive.js";
...
export default {
  ...
  methods:{
    ...
    async loadDiscounts(page = 1) {
      try {
        const result = await getDiscountList(page);
        this.dataDiscounts = result.data.data;
        this.allDiscounts = result.data.data;
